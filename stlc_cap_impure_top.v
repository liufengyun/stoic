...
Inductive sub: typ -> typ -> Prop :=
  ...
  | sub_top_pure    : forall T, closed_typ T = true -> sub T typ_top_pure
...
Axiom typing_equiv_top_pure : forall E S T t,
  E |= t ~: typ_arrow_closed typ_top_pure (typ_arrow S T) ->
  E |= t ~: typ_arrow_closed typ_top_pure (typ_arrow_closed S T).
...
Inductive capsafe: typ -> Prop :=
 ...
 | capsafe_top_pure: capsafe typ_top_pure
...
Lemma closed_typ_not_top: forall T, closed_typ T = true -> T <> typ_top.
Proof. introv Closed. introv Hc. substs. false. Qed.
...
Lemma sub_eff_eq: forall S, sub S typ_eff -> S = typ_eff.
Proof. intros. inductions H; auto. Qed.
...
Proof. intros. inductions H; try solve [intros Hc; substs; false]; auto. Qed.

Lemma sub_top_false: sub typ_top typ_top_pure -> False.
Proof. introv Sub. inductions Sub; auto. destruct T; auto.
  lets: sub_base_eq Sub1. false.
  lets: sub_eff_eq Sub1. false.
  lets: sub_arrow_inv Sub1. destruct H as [S1 [S2 [H _]]]. destruct H; false.
  lets: sub_arrow_closed_inv Sub1. destruct H as [S1 [S2 [H _]]]. inversion H.
Qed.

Lemma sub_eff_pure_false: sub typ_eff typ_top_pure -> False.
Proof. introv Sub. inductions Sub; auto. destruct T; auto.
  false* sub_top_false.
  lets: sub_base_eq Sub1. false.
  lets: sub_arrow_inv Sub1. destruct H as [S1 [S2 [H _]]]. destruct H; false.
...
Lemma sub_arrow_pure_false: forall S T, sub (typ_arrow S T) typ_top_pure -> False.
Proof. introv Sub. inductions Sub; auto. destruct T0; auto.
  false* sub_top_false.
  lets: sub_base_eq Sub1. false.
  lets: sub_eff_eq Sub1. false.
  eapply IHSub2. reflexivity. auto.
  lets: sub_arrow_closed_inv Sub1. destruct H as [S1 [S2 [H _]]]. inversion H.
Qed.

Lemma sub_top_pure_closed: forall S, sub S typ_top_pure -> closed_typ S = true.
Proof. introv Sub. inductions S; auto.
  false* sub_top_false.
  false* sub_eff_pure_false.
  false* sub_arrow_pure_false.
Qed.

Lemma sub_closed: forall S T, sub S T -> closed_typ T = true -> closed_typ S = true.
Proof. introv Sub Closed. destruct T; auto.
  apply* sub_top_pure_closed.
  lets: sub_base_eq Sub. substs*.
  lets: sub_arrow_closed_inv Sub. destruct H as [S1 [S2 [H _]]]. substs*.
Qed.
...
Proof. intros. induction H0; simpls; tryfalse.
  ...
  inversion H.
  apply typing_sub with S; auto. apply IHtyping; auto. eapply sub_closed; eauto.
Qed.
...
Proof. intros E t T1 T2 H Hc Ha. inductions Ha.
  ...
  forwards~ : typing_equiv_top_pure H0. apply* typing_app.
