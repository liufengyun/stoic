(***************************************************************************
* STLC + capabilities + subtyping w/o =>, based on formalization           *
...
Lemma sub_stoic_inv: forall T U1 U2,
  ...
  exists S1 S2, T = typ_stoic S1 S2.
...
Lemma sub_stoic_inv_sub: forall S1 S2 U1 U2,
  ...
  sub U1 S1 /\ sub S2 U2.
Proof. intros. gen_eq T1: (typ_stoic S1 S2). gen_eq T2: (typ_stoic U1 U2).
  gen S1 S2 U1 U2. inductions H; intros; substs; tryfalse; auto.
  inversions* H1.
  lets: sub_stoic_inv H0. destruct H1 as [M1 [M2 H1]]. substs.
...
Proof. intros. inductions H0; auto.
  inversions* H.
    destruct* (capsafe_decidable T1). false* capsafe_not_caprod.
...
Lemma typing_inv_abs : forall E S1 e1 T,
  ...
  forall U1 U2, sub T (typ_stoic U1 U2) ->
     sub U1 S1
...
Proof.
  ...
  destruct* (sub_stoic_inv_sub Sub). split*.
...
Lemma typing_abs_shape : forall E S e T,
  typing E (trm_abs S e) T -> T <> typ_eff /\ T <> typ_base.
Proof. intros. inductions H; try solve [split; discriminate].
  destruct T; try solve [split; discriminate].
    lets*: sub_base_eq H0.
    lets*: sub_eff_eq H0.
Qed.
...
Proof.
  introv Typ. gen t'.
  induction Typ; intros t' Red; try solve [inversions Red]; jauto.

  (* app *)
  inversions Red; try solve [apply* typing_app].
  forwards~ : typing_inv_abs Typ1. destruct H. destruct H0 as [S2 [L H0]].
    pick_fresh x. rewrite* (@subst_intro x). lets: typing_sub Typ2 H.
    apply_empty* typing_subst. forwards~ : H0 x. destruct* H4.
Qed.
