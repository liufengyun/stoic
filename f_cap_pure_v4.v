(***************************************************************************
* System-F pure with capabilities in a different universe                  *
...
Inductive primitive: env -> Prop :=
  ...
  | primitive_tvar: forall X E b, primitive E -> primitive (E & (X \at b))
  | primitive_typ: forall x X E b, primitive E -> primitive (E & x ~: (typ_fvar b X)).

Inductive inhabitable: env -> Prop :=
  | inhabitable_empty: inhabitable empty
  | inhabitable_tvar: forall X E b,
                        inhabitable E ->
                        inhabitable (E & (X \at b))
...
Inductive healthy: env -> Prop :=
  ...
  | healthy_typ: forall x E T, capsafe T -> healthy E ->
                               healthy (E & x ~: T)
  | healthy_tvar: forall X E b, healthy E ->
                                healthy (E & ( X \at b)).
...
Fixpoint subst_ee (z : var) (u : trm) (e : trm) {struct e} : trm :=
  ...
  | trm_tabs b e1 => trm_tabs b (subst_ee z u e1)
...
Proof.
  ...
  pick_fresh X. apply* (@open_tt_rec_type_core T2 0 (typ_fvar b X)).
Qed.
...
Proof.
  ...
  unfolds open_te. pick_fresh X.
   apply* (@open_te_rec_type_core e1 0 (typ_fvar b X)).
Qed.
...
Proof.
  ...
  unfolds open_te. pick_fresh X.
   apply* (@open_ee_rec_type_core e1 0 (typ_fvar b X)).
Qed.
...
Lemma subst_ee_open_te_var : forall z u e X b, term u ->
  (subst_ee z u e) open_te_var b \with X = subst_ee z u (e open_te_var b \with X).
Proof.
...
Lemma typ_all_open_tt_type: forall T U b, type (typ_all b T) -> type U -> type (open_tt T U).
...
Proof.
  ...
  autos* wft_type. rewrite* (@subst_tt_intro X T U b).
...
Lemma okt_push_inv : forall E X B,
  okt (E & X ~ B) -> exists T b, B = bind_tvar b \/ B = bind_typ T.
Proof.
  introv O. inverts O.
    false* empty_push_inv.
    lets (?&?&?): (eq_push_inv H). subst. exists* (typ_fvar b X).
    lets (?&?&?): (eq_push_inv H). subst. exists* T false.
Qed.

Lemma okt_push_tvar_inv : forall E X b,
  okt (E & (X \at b)) -> okt E /\ X # E.
Proof.
  introv O. inverts O.
    false* empty_push_inv.
...
Proof.
  ...
  lets(T & b & [H | H]): (okt_push_inv Okt); subst.
...
Proof.
 introv O W. induction F using env_ind.
  ...
  rewrite map_push. rewrite concat_assoc in *.
   lets (U&b0&[?|?]): okt_push_inv O; subst.
     lets*: (okt_push_tvar_inv O). simpls*.
     lets*: (okt_push_typ_inv O).
      applys~ okt_typ; autos*.
Qed.
...
Lemma notin_fv_tt_open : forall Y X T b,
  X \notin fv_tt (T open_tt_var b \with Y) ->
  X \notin fv_tt T.
Proof.
...
Proof. intros T Ht Hd. inductions T; auto.
  inversions Ht.
  cases* b.
...
Lemma subst_tt_type_type_0: forall P Q T Z b,
  degree_typ T = 0 -> type P -> type Q ->
  type (subst_tt Z b P T) -> type (subst_tt Z b Q T).
...
Lemma subst_tt_type_type_k: forall P Q T Z k b,
  degree_typ T <= k  -> type P -> type Q ->  type (subst_tt Z b P T) -> type (subst_tt Z b Q T).
Proof. intros. gen T. inductions k; intros.
  (* k = 0*)
  ...
  lets: le_S_n H. inversions H2.
    ...
    forwards~ : H5 X.
    unfolds open_tt.
    replace (typ_fvar b0 X) with (subst_tt Z b Q (typ_fvar b0 X)) by
       (rewrite* subst_tt_fresh; simpls; autos).
    replace (typ_fvar b0 X) with (subst_tt Z b P (typ_fvar b0 X)) in H2 by
       (rewrite* subst_tt_fresh; simpls; autos).
    rewrite <- subst_tt_open_tt_rec in *; auto.
    apply* IHk. rewrite* <- degree_typ_eq_open_tt_rec.
Qed.

Lemma subst_tt_type_type: forall P Q T Z b,
  type P -> type Q ->  type (subst_tt Z b P T) -> type (subst_tt Z b Q T).
...
Lemma same_cap_subst_tt_0: forall T Z P Q b,
  degree_typ T = 0 -> same_cap P Q -> same_as_cap (subst_tt Z b P T) (subst_tt Z b Q T).
Proof. intros.  inductions T; unfolds; autos.
  simpls; splits; cases_if*; unfolds same_cap; intros.
    destruct* H0. false* (capsafe_not_caprod H2).
  ...
  forwards~ : IHT1 Z H0. lets*: degree_typ_parent_zero H.
    ...
    splits; intros. inversions H5; simpl.
      apply capsafe_eff_any; autos* subst_tt_type_type.
      apply capsafe_any_safe; autos* subst_tt_type_type.
    inversions H5. apply* caprod_safe_eff.
  simpls. inversions H.
Qed.

Lemma same_cap_subst_tt_k: forall T Z P Q k b,
  degree_typ T <= k -> same_cap P Q -> same_as_cap (subst_tt Z b P T) (subst_tt Z b Q T).
Proof. intros. gen T. inductions k; intros.
...
Lemma same_cap_subst_tt: forall T Z P Q b,
  same_cap P Q -> same_as_cap (subst_tt Z b P T) (subst_tt Z b Q T).
Proof. intros. apply* same_cap_subst_tt_k. Qed.

Lemma capsafe_subst_tt_caprod: forall T Z P Q b,
  caprod P -> caprod Q -> capsafe (subst_tt Z b P T) -> capsafe (subst_tt Z b Q T).
Proof. intros. forwards~ : same_cap_subst_tt T Z P Q b. unfolds*. destruct* H2. Qed.

Lemma capsafe_subst_tt_capsafe: forall T Z P Q b,
