...
Inductive typ       : Set   :=
  ...
  | typ_stoic       : typ -> typ -> typ       (* stoic term abstraction *)
  | typ_all         : typ -> typ.             (* stoic type abstraction *)
...
Fixpoint pure(E: env) := match E with
  ...
  | cons (x, bind_typ T) E' => if pure_typ T then
...
Lemma pure_regular: forall E x T, binds x (bind_typ T) (pure E) -> pure_typ T = true.
Proof. intros. inductions E.
  ...
  cases* (pure_typ t). destruct (binds_push_inv H); destruct H0.
    inversions* H1.
    apply* IHE.
Qed.
...
Lemma pure_binds_in: forall E x T, pure_typ T = true ->
...
Proof. intros. induction E.
  ...
  destruct a. destruct b.
    ...
    destruct H1. destruct* (pure_typ t).
Qed.
...
Lemma pure_single_true : forall x U, pure_typ U = true ->
  pure (x ~: U) = x ~: U.
Proof. intros.
...
Lemma pure_single_false : forall x U, pure_typ U = false ->
  pure (x ~: U) = empty.
Proof. intros.
  replace (x ~: U) with (empty & x ~: U) by rewrite* concat_empty_l.
  rewrite <- cons_to_push. simpls. rewrite H.
...
Proof. intros. induction* E.
  ...
  destructs (okt_push_typ_inv H). destruct* (pure_typ t).
...
Lemma pure_subst_tt: forall T,
  pure_typ T = true ->
  forall Z P, pure_typ (subst_tt Z P T) = true.
...
Proof. intros. inductions E.
  ...
  (* x:T*)
    ...
    simpl. cases (pure_typ (subst_tt Z P t)).
...
Proof.
  ...
  apply_fresh* typing_tabs as Y.  destruct (typing_regular TypU).
    ...
    (* if U is pure, then use IH; else  x is free in e1; *)
    repeat(rewrite pure_dist in *). remember (pure_typ U) as b. destruct b.
      (* pure_typ U = true *)
  ...
  apply* typing_tapp.
  apply* typing_stoic. rewrite ?pure_dist in *. remember (pure_typ U) as b. destruct b.
...
Lemma typing_stoic_inv: forall t E S T,
  typing E (trm_abs S t) (typ_stoic S T) ->
  (exists L, forall x, x \notin L -> typing ((pure E) & x ~: S) (t open_ee_var x) T).
Proof.
  intros. inversions H. apply* typing_abs_inv.
Qed.
...
Proof.
  ...
  apply* typing_stoic.
  (* case: poly *)
  inversions Red; try solve [ apply* typing_poly ].
  lets*: typing_regular Typ2.
  forwards*: typing_abs_regular Typ1. substs.
  forwards*: typing_stoic_inv Typ1.
  destruct H1 as [L Y]. pick_fresh x. forwards*: Y x.

  rewrite* (@subst_ee_intro x).
  apply_empty typing_through_subst_ee; substs*.
  apply* typing_stoic. apply* okt_typ. lets*: typing_wft Typ2.
  rewrite pure_dist, pure_single_true; auto.
Qed.
...
Proof.
  ...
  right. destruct* IHTyp1 as [Val1 | [e1' Rede1']].
    destruct* IHTyp2 as [Val2 | [e2' Rede2']].
    lets* Typ3: typing_degen Typ1.
    destruct (canonical_form_abs Val1 Typ3) as [V [e3 EQ]].
      subst. exists* (open_ee e3 t2). apply* red_abs. lets*: typing_regular Typ1.
    exists* (trm_app e1' t2). apply* red_app_1. lets*: typing_regular Typ2.
Qed.
...
Proof. introv H. inductions E; simpls*.
  ...
  inversions H. rewrite empty_def in H1. inversion H1.
    rewrite <- cons_to_push in H0. inversions H0.
    cases* (pure_typ t). apply* healthy_typ.
...
Proof. introv Deg H Typ. inductions Typ; intros; autos.
  ...
  simpl in Deg. destruct (max_zero_inv _ _ Deg).
    forwards~ : IHTyp1. forwards~ : IHTyp2.
    inversions* H3. false. autos* capsafe_not_caprod.
    inversion* H8.
Qed.
...
Proof. introv Deg H Typ. gen t E T. inductions k; intros.
  ...
  simpl in Deg. forwards~ LT1: Max.max_lub_l Deg. forwards~ LT2: Max.max_lub_r Deg.
    forwards~ : IHTyp1. forwards~ : IHTyp2.
    inversions* H1. false. autos* capsafe_not_caprod.
    inversion* H6.
Qed.
